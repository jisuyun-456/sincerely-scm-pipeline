...
if platform.system() == "Windows":
    ...
    # Linux (GitHub Actions — apt install fonts-nanum)
...
# ── 데이터 조립 ───────────────────────────────────────────────────────────────
def download_qr_image(url: str) -> bytes | None:
    if not url:
        return None
    try:
        ...
        print(f"  ⚠️ QR 이미지 다운로드 실패: {e}")
        return None


def fetch_shipments(filter_date: str | None, sc_id: str | None,
                    record_id: str | None = None) -> list[dict]:
    if record_id:
        # Airtable record ID 직접 조회 (Make/GitHub Actions 버튼 트리거용)
...
def _draw_info_block(c: rl_canvas.Canvas, y: float, doc: dict,
    ...
    # 배경 rect를 block_h 높이로 먼저 채워 두 블록 높이를 시각적으로 맞춤
    c.setFillColor(colors.HexColor("#DBEAFE"))
    c.rect(MARGIN, y - block_h, HALF, block_h, fill=1, stroke=0)
    c.setFillColor(colors.HexColor("#DCFCE7"))
    c.rect(MARGIN + HALF + 2 * mm, y - block_h, HALF, block_h, fill=1, stroke=0)

    # 각 테이블은 top-aligned 으로 그림
    tbl_send.drawOn(c, MARGIN, y - hs)
    tbl_recv.drawOn(c, MARGIN + HALF + 2 * mm, y - hr)
...
def _draw_signature_block(c: rl_canvas.Canvas, y: float,
                          ...
                          qr_bytes: bytes | None = None) -> float:
    QR_SIZE = 26 * mm
    style = [
        ("BOX",           (0, 0), (-1, -1), 0.5, COLOR_BORDER),
        ("GRID",          (0, 0), (-1, -1), 0.4, COLOR_BORDER),
    ...
    if qr_bytes:
        qr_img  = RLImage(io.BytesIO(qr_bytes), width=QR_SIZE, height=QR_SIZE)
        rem_w   = INNER_W - QR_SIZE
        tbl = Table(
            [[_para("상품 및 박스 수량을 확인하고 서명합니다.", font_bold, 9),
              qr_img,
              _para("(서명)  _________________", font, 9, align="RIGHT")]],
            colWidths=[rem_w * 0.62, QR_SIZE, rem_w * 0.38],
            rowHeights=[QR_SIZE + 8 * mm],
        )
    else:
        tbl = Table(
            [[_para("상품 및 박스 수량을 확인하고 서명합니다.", font_bold, 9),
    ...
    h = _draw_table(c, tbl, MARGIN, y, INNER_W, 40 * mm)
    return y - h - 3 * mm
...
def _draw_items_table(c: rl_canvas.Canvas, y: float, doc: dict,
    ...
    # 최소 6행 보장 (비고 행 제외 카운트)
    real_count = sum(1 for itm in items if not itm.get("is_note"))
    while real_count < 6:
        items.append({"no": real_count + 1, "pt": "", "name": "", "qty": "",
                      "ordered_qty": "", "is_note": False})
        real_count += 1

    # 헤더: No | 품목명 | 주문 수량 | 출고 수량 | 확인□
    COL_W = [10*mm, 110*mm, 24*mm, 24*mm, 12*mm]
    header = [
        _para("No",       font_bold, 8.5, COLOR_WHITE, "CENTER"),
        _para("품목명",    font_bold, 8.5, COLOR_WHITE, "CENTER"),
        _para("주문 수량", font_bold, 8.5, COLOR_WHITE, "CENTER"),
        _para("출고 수량", font_bold, 8.5, COLOR_WHITE, "CENTER"),
        _para("확인",      font_bold, 8.5, COLOR_WHITE, "CENTER"),
    ]

    COLOR_NOTE_BG = colors.HexColor("#F0F0F0")
    COLOR_NOTE_FG = colors.HexColor("#6B7280")

    rows = [header]
    note_rows: list[int] = []  # 비고 행 인덱스 (헤더=0 기준)
    for itm in items:
        row_idx = len(rows)
        if itm.get("is_note"):
            ...
            rows.append([
                _para("", font, 8),
                _para(f"  └ {itm['name']}", font, 8, COLOR_NOTE_FG),
                _para("", font, 8),
                _para("", font, 8),
                _para("", font, 8),
            ])
    ...
    # 비고 행: 회색 배경, 전체 열 병합 해제 + 색상 적용
    for ri in note_rows:
        cs += [
            ("BACKGROUND",    (0, ri), (-1, ri), COLOR_NOTE_BG),
...
def draw_confirmation(c: rl_canvas.Canvas, doc: dict, font: str, font_bold: str) -> None:
    ...
    qr_bytes = download_qr_image(doc.get("qr_url", ""))
    ...
    y = _draw_signature_block(c, y, font, font_bold, qr_bytes=qr_bytes)
...
def main():
    ...
    parser.add_argument("--record-id", help="Airtable record ID 단건 (예: recXXXXXX) — Make/GitHub Actions 버튼 트리거용")
    ...
    # ── Shipment 조회 ─────────────────────────────────────────────────────────
    print("▶ Shipment 조회 중…")
    recs = fetch_shipments(args.date, args.sc_id, getattr(args, "record_id", None))
