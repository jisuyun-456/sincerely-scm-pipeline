...
# 4. 핵심 분석 함수
# ═══════════════════════════════════════════════════════
def analyze(records: list[dict], live_cbm: dict, week_start: date) -> dict:
    """
    반환 구조:
    ...
    cbm_sources = {"manual": 0, "box_parse": 0, "estimate": 0, "none": 0}
    ...
    for rec in records:
        ...
        if total_cbm_field and total_cbm_field > 0:
            ...
            if item_str.strip():
                cbm_val = estimate_cbm_from_items(item_str)
                if cbm_val > 0:
                    cbm_sources["estimate"] += 1
                else:
                    cbm_sources["none"] += 1
            else:
                cbm_sources["none"] += 1
...
def build_trend(prev_records: list[dict], live_cbm: dict) -> list[dict]:
    """최근 4주 주간 손익 트렌드"""
...
def build_slack_blocks(
    ...
    for i, (name, qty, cbm) in enumerate(this["top_items"][:6], 1):
        item_lines += f"  {i}. {name}  `{qty:,}개`  →  *{cbm:.3f}m³*\n"
...
def main():
    ...
    this_data = analyze(this_recs, live_cbm, this_mon)
    next_data = analyze(next_recs, live_cbm, next_mon)
    trend     = build_trend(prev_recs, live_cbm)
