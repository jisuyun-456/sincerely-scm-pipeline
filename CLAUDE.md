...
| 키워드 | 에이전트 | 모델 |
|--------|---------|------|
| 품목코드, 로케이션, 공급사, 바코드, ROP, BIN | wms-master-data (SK-01) (운영 데이터 조회·설정. Airtable 스키마 변경은 scm-airtable-pattern 스킬 병행) | sonnet |
...
<!-- Agent: .claude/agents/*.md (Claude-managed workers) | Plugin/Skill: ~/.claude/commands/ (user-managed tools) -->
| 키워드 | 도구 |
|--------|------|
...
- **갱신 책임**: 태스크 완료 시 Claude가 status를 "done"으로, 신규 발견 이슈는 즉시 append
- **갱신 시점**: 각 세션 종료 전, 미션 Sprint 완료 시, audit 결과 반영 시
- **우선순위 기준**: critical(P0) > high(P1) > medium(P2) > low > done

## 검증 체크포인트 (코드 수정/생성 후 필수)
1. **Validation Contract 확인** (기능 수정/확장·새 기능 시): Phase 2에서 선언한 통과 조건 N개 각각에 대해 증거 수집
   Contract 파일 위치: `~/.claude/harness/logs/<mission-id>/contract.yaml` | 포맷 참조: `~/.claude/harness/event-log-spec.md`
