Three edits are justified by the top-3 retrieved skills:

1. **`linked-record-id-over-text-match` (11)** — Red Flag table has no guard against text-based carrier filtering. Add one row.
2. **`tms-cost-lane-anomaly-detection` (11)** — §1 "Lane별 비용 분석" doesn't mention the 6-section monthly report structure that's clearly the standard output. Add one bullet.
3. **`carrier-npv-dual-breakeven` (9)** — NPV/breakeven for lane evaluation isn't in the routing table at all, creating ambiguity. Add one row to clarify it stays in SK-09.
...
| "이 carrier를 이름으로 필터하면 됨" | Airtable 파트너는 **linked record_id로 필터** — 표시 이름은 언제든 변경 가능. `PARTNER_RECORD_ID = "recXXX"` 방식만 사용. 텍스트 매칭 금지. |
...
1. **Lane별 비용 분석**
   ...
   - CBM당 단가 랭킹 (높은 순)
   - 월별 추이 (최근 13주 기준)
   - 월간 리포트 표준 6섹션: ①총 운임 합계 ②Carrier별 분해(share%) ③Lane Top10 비용 ④모드별 비중(Parcel/LTL/FTL) ⑤MoM 증감 ⑥예산 대비 편차(>10% → 빨간색 플래그)
...
| "신규 carrier NPV·브레이크이븐 분석" | SK-09 (이 에이전트) | lane 비용 재무 모델링 (계약 협상이 아닌 분석) |
