Analyzing the skill bundle against the current agent.md to identify gaps, then producing the candidate.

**Edit rationale (3 edits, each traced to a skill):**

1. **Step 1.5 added** — `vendor-sku-barcode-mismatch-resolution` (r=20): barcode/SKU mismatch handling is a complete gap; no mention anywhere in current agent.
2. **Step 3 extended** — `asn-gr-quantity-discrepancy-procedure` (r=20): current text says "수량" but doesn't specify actual vs ASN qty or the 4h discrepancy-note SLA.
3. **Step 4 extended** — `multi-supplier-dock-slot-scheduling` (r=20): current text only says "record it"; skill establishes D2S ≤2h target and >4h root-cause trigger.
...
2. **바코드·SKU 매칭 (inbound scan)**
   - 공급사 바코드 스캔 → `barcode_master`에서 내부 SKU 조회
   - 매핑 존재 → GR 진행 / 불일치 → `vendor_sku_map` 재조회
     - 매핑 발견: auto-resolve + auto_match 이벤트 로그
     - 매핑 없음: `manual_confirm_queue` INSERT + 작업자 알림 → 확인 후 `vendor_sku_map` INSERT (학습)
   - 공급사당 분기 라벨 오류 ≥ 3건 → `vendor.compliance_flag = 'WARNING'` + 구매팀 통보
3. **AQL 샘플 검사**
   - 로트 크기 → AQL 표 → 샘플 수 결정
   - 합격: GR 진행 / 불합격: NCR 발행 후 SK-07 wms-return으로 위임 (NCR INSERT는 SK-07이 수행, SK-02는 트리거만)
4. **GR 처리 (movement INSERT)**
   - **실수령 수량으로만 GR 처리** — ASN 수량을 그대로 사용 금지
   - 이동유형 101 / 실수령 수량 / 로케이션 / LOT 번호 / 일자 → mcp__scm_airtable__wms_movements 호출
   - ASN 수량 ≠ 실수령 수량 시: GR 후 **4h 이내** 불일치 노트 발행, 공급사 24h 응답 SLA 설정
     - 해소 경로: (A) 운송사 과실 → 데빗 메모 / (B) 공급사 포장 오류 → 보정 ASN 또는 크레딧 / (C) 72h 미해소 → 구매 담당자 에스컬레이션
5. **Dock-to-Stock 시간 기록**
   - 도착 timestamp ~ 입고 완료 timestamp → KPI 필드
   - **목표: D2S ≤ 2h** / D2S > 4h 건 → 원인 분석 필수 (담당자·공급사·SKU 축 분해)
6. **불합격 시 LOT 격리**
