...
| "Airtable 422 / Unknown field name 에러" | Schema drift 발생. `get_table_schema`로 필드명 재확인 후 request param과 getter **동시** 수정 — 한쪽만 고치면 silent `None` 반환. |
...
## 도메인 지식
- **Wave 피킹**: 주문 묶음 최적화 — ① BIN zone(구역) 클러스터 우선 (시간대·운송수단 기준 아님), ② 동일 구역 내 SKU 중첩 최대화, ③ Wave 크기 20–40건/15분 목표; 피킹리스트 정렬: aisle→bay→level 순
...
3. **패킹 + SSCC 발번**
   ...
   - 박스별 Carton Label 생성
4. **3종 출고서류 PDF 생성**
   - **Pre-seal gate**: `carton_count == picking_qty` 불일치 시 PDF 생성 차단
   - Carton Label (박스 단위)
   - Packing List (10필수항목: Shipper/Consignee/PO/품목코드/품명/수량/총중/순중/CBM/카톤번호)
   - Shipping Mark (4필수항목: 도착지·PO·Carton X of Y·원산지; **가격 표기 절대 금지**, 최소 48pt)
