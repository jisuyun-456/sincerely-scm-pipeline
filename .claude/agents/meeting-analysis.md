...
출력: docs/briefs/{YYMMDD}_물류파트_회의록 (문서형).html"
)
```

- doc-brief는 `.claude/agents/doc-brief.md`의 회의록 모드 매핑을 따라 자동 변환한다.
- 생성된 HTML 경로: `c:\Users\yjisu\Desktop\SCM_WORK\docs\briefs\{YYMMDD}_물류파트_회의록 (문서형).html`

### Step 2 — Obsidian 저장 (회의록 요약 + HTML 링크)

HTML 생성 완료 확인 후 `mcp__obsidian__obsidian_append_content`로 Vault에 저장한다.

- **저장 경로 (Vault 상대 경로):** `SCM/Meetings/{YYMMDD}_물류파트_회의록.md`
  예: `SCM/Meetings/260506_물류파트_회의록.md`
- **내용 템플릿:**
...
- **문서형 브리프:** [HTML 브리프 열기](file:///C:/Users/yjisu/Desktop/SCM_WORK/docs/briefs/{YYMMDD}_물류파트_회의록%20(문서형).html)
```

**링크 포맷:** 파일명 공백만 `%20`으로 치환한다. 한글은 그대로 유지해도 Obsidian이 처리한다.
예: `260506_물류파트_회의록 (문서형).html`
→ `file:///C:/Users/yjisu/Desktop/SCM_WORK/docs/briefs/260506_물류파트_회의록%20(문서형).html`
