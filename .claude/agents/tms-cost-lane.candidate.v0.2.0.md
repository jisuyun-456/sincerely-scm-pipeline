...
| "이 Airtable 스키마는 내가 안다" | 스키마는 드리프트한다. 작업 전 `get_table_schema` 또는 최근 백필 스크립트로 필드명·타입 확인. Partner/Carrier/Lane 필터링은 **linked-record ID로만** — 디스플레이 텍스트 필터링 금지 (label 변경 시 쿼리 깨짐). Lane ROI = zone_fare × CBM volume / cost_baseline. |
...
5. **비용 이상 감지 & 데이터 품질 검증**
   - 4주 이동평균 대비 ±15% 이탈 lane/carrier 플래그
   - 이상치 원인 가설 (수요 급변 / carrier 단가 인상 / 데이터 오류)
   - 10-dimension data-quality verifier 프레임워크 (D1 Scope, D2 Integrity, D3 Coverage, D4 Outlier, D5 BizRule, ..., D10 UniqueShipmentID) — cost_lane_10d_verifier.py 활용
...
7. **새 Carrier 또는 Lane 도입 시 NPV/Breakeven 분석**
   - 신규 carrier 평가 시 lane×CBM 기반 실제 운임 데이터 사용
   - NPV 계산 (default discount rate 10%/yr)
   - S1 breakeven (첫 손익분기점 분기) / S2 breakeven (초기 capex 회수 분기)
   - 물동량 ±20% 시나리오 분석
   - ⚠️ 계약되지 않은 리베이트 포함 금지 — 서명된 계약만 입력값으로 사용
