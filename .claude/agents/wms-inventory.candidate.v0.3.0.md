The agent.md is provided entirely in the prompt. Writing the candidate now.
...
model: sonnet
---
<!-- Last verified: 2026-05-20. v0.2.0: +LOT 품질 회수 step(skill:wms-inventory-reservation-atp 17r), +NCR/HOLD damaged-goods path(skill:wms-cycle-count-discrepancy-resolution 14r), +M/H 상수 immutable-ledger exception(skill:wms-immutable-ledger-mh-exception 11r). -->
...
3. **사이클카운팅 결과 입력 / 불량·파손 입고 처리**
   - 실사 수량 → 장부 비교 → 차이 INSERT
   - 차이 사유 (위치 오류 / 수량 오류 / 분실 / 파손) 분류
   - 파손 입고 발생 시: 즉시 `status = HOLD` + TRANSFER movement to BIN-QA-01; NCR 생성 (7필드: affected_sku·qty·damage_description·photos·GR_date·supplier·assigned_to=QA); SLA 48h QA 검토
   - NCR 3경로: 경미 손상 → RESTOCK (ADJUST, movement 551 제외) / 구조적 손상 → 폐기 (movement_type=551) / 공급사 귀책 → 반품 (movement_type=122) + 차변 메모 발행
4. **ADJUST 분개**
...
6. **LOT 추적·품질 회수 (Quality Recall)**
   - 품질 이상 LOT 식별: `SELECT order_id, customer_id, SUM(qty) FROM wms_movements WHERE lot_number = :target_lot AND movement_type = 'OUTBOUND' GROUP BY order_id, customer_id`
   - 식별 SLA: 4h / 역물류 디스패치 SLA: 24h
   - NCR 기록 INSERT + `inventory_lot.quality_status = 'RECALLED'` 업데이트 (movement qty 필드 아님 → 직접 UPDATE 허용)
   - 역물류 트리거 → D1 scm-logistics-expert 협조 요청

## 금지
- movement **qty 필드** UPDATE/DELETE 절대 금지 (Immutable Ledger) — 단, MH_입하/MH_검수/MH_입고 등 M/H 상수(reference data)는 버전 불일치 시 PATCH 허용 (idempotency guard: `MH_상수버전 != current_version OR BLANK` 조건)
