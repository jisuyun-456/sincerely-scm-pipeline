...
- **자체기사 피로도 기준 (6주 롤링)**: op_days ≥ 30일(rest_ratio < 28%), max_streak ≥ 14일 연속, overbooking_count > 0 → 즉시 검토 대상. 배차 요청 시 CBM 체크와 함께 사전 확인 필수.
- **Carrier 마감시간(Cutoff)**: 로젠 14:00(내부마감 12:00), GogoX 16:00(14:00), A-One 15:00(13:00). wave_commit_deadline = carrier.cutoff − 2h. 내부마감 초과 미출하 → 익영업일 자동 재배차 + 고객 지연 알림 트리거.
...
- Carrier 청구서 수취 시 shipment_id별 운임 INSERT; 아웃소스 판별은 `F_PROJECT_CODE` MM-prefix 기준(`partner_link` 유무로 판별 금지 — MM 프로젝트는 partner_link 없이도 outsource일 수 있어 silent misclassification 발생), 분개 검증은 D2 tax-accounting-expert
...
<!-- candidate: v0.2.0 -->
<!-- edits:
  1. 도메인 지식 +2 lines — "자체기사 피로도 기준": traces to driver-fatigue-six-week-analysis (20 retrievals); alert thresholds (op_days/max_streak/overbooking_count) were absent from the agent, causing repeated skill lookups.
  2. 도메인 지식 +2 lines — "Carrier 마감시간(Cutoff)": traces to pod-collection-72h-verifier (20 retrievals); cutoff table and wave_commit_deadline formula were not represented anywhere in the agent.
  3. 협조 위임 carrier line — expanded with MM-prefix anti-pattern: traces to tms-settlement-outsource-mm-prefix (20 retrievals); partner_link-based detection causes silent in-house misclassification on MM projects.
-->
