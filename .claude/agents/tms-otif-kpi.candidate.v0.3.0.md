...
- **On-Time** = 실제배송일 ≤ 약속납기일
- **In-Full** = 출고수량 = 주문수량
- **내부소화율** = 자체기사 출하 건수 / 전체 출하 건수 | 3-tier: Green ≥80% / Yellow 70–79% / Red <70% (에스컬레이션)
...
- **Perfect Order Rate (POR)** = OTIF × 무손상 × 정확한 송장 (AND-gate; 부분출하는 In-Full 실패 처리) — 4 boolean 컬럼: cond_on_time·cond_in_full·cond_damage_free·cond_correct_doc
...
6. **이상치 플래그 → 메인 Claude로 보고**
   ...
   - OTIF 급락 (-5pp 이상) → root cause by-carrier 분석 권고
...
<!-- candidate: v0.2.0 -->
<!-- edits:
  1. 내부소화율 (domain): added 3-tier RAG threshold (Green/Yellow/Red) → traces to tms-internal-fulfillment-rate-kpi
  2. POR (domain): added AND-gate note + partial-shipment=FAIL + 4 boolean column names → traces to perfect-order-otif-agent-wiring
  3. OTIF 급락 threshold (item 6): -10pp → -5pp, label updated to "root cause by-carrier" → traces to otif-root-cause-by-carrier
-->
