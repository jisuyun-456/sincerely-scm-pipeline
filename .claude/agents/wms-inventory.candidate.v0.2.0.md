...
|---|---|
| "그냥 작은 데이터 수정인데" | SCM 데이터 = **Immutable Ledger**. movement/mat_document INSERT ONLY. 정정은 storno(역분개) 또는 보정 레코드로만 — UPDATE/DELETE 금지. **단, M/H 표준 상수(MH_입하·MH_검수·MH_입고)는 reference 정의이므로 `MH_상수버전` 변경 시 PATCH 허용 — 이동수량 필드는 예외 없이 INSERT ONLY.** |
...
6. **LOT 추적 / 리콜**
   - 대상 LOT의 OUTBOUND movement → order_id·customer_id 역추적: `SELECT order_id, customer_id, SUM(qty) FROM wms_movements WHERE lot_number = :lot AND movement_type = 'OUTBOUND' GROUP BY order_id, customer_id`
   - 리콜 대상 확정 후 TMS 역물류 트리거 (식별 SLA <4h, 역배송 <24h)
   - 완료 후 NCR INSERT + `inventory_lot.quality_status = 'RECALLED'` UPDATE
7. **불량입하 / Hold·NCR 처리**
   - `status = HOLD` 즉시 설정 → BIN-QA-01 TRANSFER movement INSERT (wave planning 자동 제외)
   - NCR 7필드 기록: affected_sku | qty | damage_description | photos | GR_date | supplier | assigned_to=QA (SLA: 48h)
   - 3-path 결정: 경미 손상 → RESTOCK(ADJUST) / 구조 결함 → DISPOSE(551) / 공급사 과실 → RETURN(122 + debit memo)
