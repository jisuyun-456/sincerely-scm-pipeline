...
1. **ASN ↔ PO 매칭**
   ...
   - 입하 스캔: supplier barcode → barcode_master 불일치 시 vendor_sku_map 조회 → 자동해소 or manual_confirm_queue INSERT. 공급사 분기별 오류 3회 이상 → compliance_flag='WARNING' 설정 후 구매 담당에 통보.
2. **AQL 샘플 검사**
   - 로트 크기 → AQL 표 → 샘플 수 결정
...
3. **GR 처리 (movement INSERT)**
   ...
   - GR qty = 실제 수령 수량만; ASN qty ≠ 실물 수량 시 GR 후 4h 이내 discrepancy note 발행, 운송사·공급사 증빙(POD/BL) 요청. 미착 수량에 대한 GR 선행 처리 금지.
4. **Dock-to-Stock 시간 기록**
   - 도착 timestamp ~ 입고 완료 timestamp → KPI 필드
5. **불합격 시 LOT 격리 및 disposition**
   - QI_HOLD 상태 → BIN-QA-01 격리; wave picking은 UNRESTRICTED 재고만 허용 (QI_HOLD 제외). QA SLA 48h.
   - 승인 시: 이동유형 311 → 일반 보관 BIN, 상태 UNRESTRICTED 전환.
   - 반품(RTV) 시: 이동유형 122 + debit memo 발행 → SK-07 wms-return 위임.
   - 폐기 시: 이동유형 551 + write-off 회계 처리 → D2 tax-accounting-expert 확인.
