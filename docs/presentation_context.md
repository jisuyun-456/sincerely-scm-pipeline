...
**AS-IS vs TO-BE 핵심 수치 비교:**

| 항목 | AS-IS | TO-BE | 개선 |
|------|-------|-------|------|
| 데이터 소스 | Airtable 2 base (비정규화) | Supabase 6스키마 51테이블 (정규화) | 데이터 무결성 확보 |
| 필드/컬럼 수 | 445개 (중복 포함) | ~350개 (중복 제거) | 21% 감소 |
| 회계 연동 | 수동 기표 (더존에 직접 입력) | 자동 전표 생성 (트리거 6개) | 완전 자동화 |
| 재고 추적 | 머릿속 계산 + rollup 버그 | 실시간 quants + stock_movements | 실시간 정확도 |
| 운영 UI | Airtable (스프레드시트형) | Retool (10페이지 대시보드) | 전용 SCM UI |
| 데이터 보호 | 누구나 수정/삭제 가능 | INSERT ONLY 불변 원장 + RLS | 감사 추적 완벽 |
| 월말 정산 | 포스트잇 모아서 수동 집계 | 버튼 하나 → 자동 마감 | 야근 제로 |
...
### 1. As-Is / To-Be 비교
- **레이아웃**: 좌우 분할(split-screen) — 왼쪽 AS-IS(약간 흐린 톤) / 오른쪽 TO-BE(밝고 따뜻한 톤)
- **수치 비교 테이블**: 카드 형태로 핵심 7개 항목 비교, 개선 폭 시각적 강조 (프로그레스바 또는 before/after)
- **워크플로우 다이어그램**: ASCII 대신 SVG 플로우차트 — 노드 hover 시 설명 팝업, 부드러운 곡선 연결선
- **AS-IS 한계점**: 부드러운 경고 아이콘 + 약간 흐린 스타일
- **TO-BE 이점**: 따뜻한 체크 아이콘 + 하이라이트 스타일
- **키 메시지**: "수동 → 자동", "야근 → 야근 제로" 같은 임팩트 문구를 크게

### 2. 마이그레이션 맵
- 인터랙티브 Sankey/트리맵 또는 테이블+매핑 화살표
- 왼쪽 Airtable 테이블 → 오른쪽 Supabase 테이블 연결선
- 신규 테이블은 별도 색상으로 구분

### 3. 스키마 탐색기
- 6스키마 아코디언/탭 + 테이블 클릭 시 컬럼 상세 펼침
- 각 스키마를 색상으로 구분 (shared=파랑, mm=초록, wms=주황, pp=보라, tms=청록, finance=빨강)
- SAP 대응 코드 배지 표시

### 4. E2E 워크플로우
- 타임라인/스텝퍼 UI, 각 단계 클릭 시 SQL/트리거 상세 표시
- 자동 트리거 발동 시 애니메이션 효과 (번개 아이콘 등)
- 단계별 데이터 흐름 시각화

### 디자인 디렉션 — Claude / Anthropic 스타일
- **톤**: 깔끔하고 따뜻하며 약간의 역동성 — Anthropic/Claude의 브랜드 느낌. 과하지 않으면서 정제된 우아함. 차분하지만 지루하지 않은 리듬감.
- **타이포**: 본문 Pretendard (또는 Noto Sans KR), 제목은 살짝 무게감 있는 세미볼드. 과한 볼드 금지. 여백으로 호흡.
- **색상**:
  - 메인: 따뜻한 크림 화이트(#faf9f7) 배경 + 차분한 다크(#1a1a2e) 텍스트
  - 액센트: Claude 오렌지-코랄(#d97706 ~ #ea580c) 포인트
  - AS-IS: 쿨그레이(#94a3b8) 톤
  - TO-BE: 따뜻한 앰버-코랄(#f59e0b ~ #ea580c) 톤
  - 보조: 부드러운 블루(#3b82f6) 링크/인터랙션용
- **배경**: 미세한 그레인 텍스처 또는 노이즈 오버레이로 따뜻함 표현. 순백색 금지.
- **모션**: 부드러운 fade-in + 약간의 slide-up. 과한 바운스 금지. ease-out 커브 위주.
- **카드/컨테이너**: 얇은 보더(1px) + 미세한 shadow. 둥근 모서리(8~12px). 배경은 반투명 white.
- **한국어**, 라이트 모드 기본 (다크 모드 토글 선택적)
- **반응형**: 데스크탑 발표 기준
