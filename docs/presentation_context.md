...
### 1. As-Is / To-Be 비교
- **레이아웃**: 좌우 분할(split-screen) — 왼쪽 AS-IS(어둡고 탁한 톤) / 오른쪽 TO-BE(밝고 선명한 톤)
- **인터랙션**: 토글 버튼으로 "실제 시스템 비교" ↔ "붕어빵 가게 비유" 전환
- **수치 비교 테이블**: 카드 형태로 핵심 7개 항목 비교, 개선 폭 시각적 강조 (프로그레스바 또는 before/after)
- **워크플로우 다이어그램**: ASCII 대신 SVG 플로우차트 — 노드 hover 시 설명 팝업
- **AS-IS 한계점**: 빨간 경고 아이콘 + 취소선 스타일
- **TO-BE 이점**: 초록 체크 아이콘 + 하이라이트 스타일
...
### 디자인 디렉션
- **톤**: 산업/유틸리티 스타일 — SCM/물류 맥락에 맞는 실용적이고 신뢰감 있는 디자인
- **타이포**: 본문 Pretendard, 강조 제목은 볼드 + 큰 사이즈
- **색상**: 네이비(#1a365d) 기반 + 주황(#ed8936) 액센트, AS-IS는 회색톤, TO-BE는 파란톤
- **한국어**, 다크/라이트 모드 토글
- **반응형**: 데스크탑 발표 기준, 태블릿에서도 볼 수 있게
