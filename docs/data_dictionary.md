...
> 6개 스키마, 55개 테이블에 대한 전체 컬럼 정의서
> 최종 갱신: 2026-04-02 | 기준: Supabase migrations 002~018
...
Materials Management + Quality Management. 구매, 입고, 재고이동, 품질검사, 반품, 폐기, 불량코드 관리, 구매단가 이력 등.
총 13개 테이블.
...
#### 5.2.11 mm.defect_catalogs (불량코드 카탈로그)

> SAP QS41 | 품질 검사에서 사용하는 구조화된 불량코드 마스터

| 컬럼명 | 데이터타입 | 한글 설명 | SAP 대응 | 예시 |
|--------|-----------|-----------|----------|------|
| id | UUID | 고유 식별자 (PK) | — | `550e8400-...` |
| defect_code | VARCHAR(20) | 불량코드 (UNIQUE) | QMGRP+QMCOD | `PRT-001`, `MFG-002` |
| defect_name | VARCHAR(200) | 불량명 | — | `인쇄 색상 불일치` |
| defect_category | VARCHAR(50) | 불량 카테고리 | Catalog Type | `printing`(인쇄), `manufacturing`(제조), `material`(자재), `dimensional`(치수), `packaging`(포장), `labeling`(라벨), `color`(색상), `contamination`(오염) |
| severity | VARCHAR(10) | 심각도 | — | `critical`(안전/법규), `major`(기능), `minor`(외관) |
| applies_to_group_id | UUID | 적용 자재그룹 (FK: material_groups) | — | 특정 자재그룹에만 적용 (NULL=전체) |
| description | TEXT | 상세 설명 | — | `인쇄 색상이 승인 샘플과 불일치` |
| is_active | BOOLEAN | 활성 여부 | — | `TRUE` |
| created_at | TIMESTAMPTZ | 생성일시 | — | `2026-04-02 09:00:00+09` |
| updated_at | TIMESTAMPTZ | 수정일시 | — | `2026-04-02 09:00:00+09` |

> 초기 시드 데이터: 포장/인쇄 업계 표준 불량코드 21종 (PRT 6종, MFG 4종, MAT 3종, PKG 2종, 라벨 2종, CTM 2종, DIM 2종)

---

#### 5.2.12 mm.inspection_defects (검사별 불량 상세)

> 검사 ↔ 불량코드 다:다 관계 (quality_inspections의 TEXT[] defect_codes를 구조화)

| 컬럼명 | 데이터타입 | 한글 설명 | SAP 대응 | 예시 |
|--------|-----------|-----------|----------|------|
| id | UUID | 고유 식별자 (PK) | — | `550e8400-...` |
| inspection_id | UUID | 검사 (FK: quality_inspections) | PRUEFLOS | 해당 검사 Lot |
| defect_catalog_id | UUID | 불량코드 (FK: defect_catalogs) | QMGRP+QMCOD | 발견된 불량 유형 |
| defect_qty | INTEGER | 불량 수량 | — | `15` |
| sample_position | VARCHAR(50) | 샘플 위치 | — | `Box-3, Row-2` |
| notes | TEXT | 비고 | — | `인쇄 색상 PANTONE 차이` |
| photo_url | TEXT | 불량 사진 URL | — | `https://...` |
| created_at | TIMESTAMPTZ | 생성일시 | — | `2026-04-02 09:00:00+09` |

> UNIQUE 제약: (inspection_id, defect_catalog_id) — 검사당 불량코드별 1건 (수량으로 합산)

---

#### 5.2.13 mm.purchasing_info_records (구매단가 이력)

> SAP ME11 | 공급사-자재별 가격 계약 및 조달 조건 이력

| 컬럼명 | 데이터타입 | 한글 설명 | SAP 대응 | 예시 |
|--------|-----------|-----------|----------|------|
| id | UUID | 고유 식별자 (PK) | — | `550e8400-...` |
| parts_id | UUID | 자재 (FK: parts_master) | MATNR | 가격 대상 부품 |
| vendor_id | UUID | 공급사 (FK: vendors) | LIFNR | 가격 제공 공급사 |
| net_price | NUMERIC(15,2) | 순단가 | NETPR | `1500.00` |
| currency_code | VARCHAR(3) | 통화 | WAERS | `KRW` |
| price_unit | INTEGER | 가격단위 | PEINH | `1`(1EA당), `100`(100EA당) |
| unit_of_measure | VARCHAR(10) | 단위 (FK: units_of_measure) | BSTME | `EA` |
| min_order_qty | INTEGER | 최소주문수량 | — | `500` |
| standard_lead_time_days | INTEGER | 표준 리드타임(일) | APLFZ | `14` |
| payment_terms | VARCHAR(50) | 결제조건 | ZTERM | `net30`, `prepaid` |
| valid_from | DATE | 유효시작일 | DATAB | `2026-01-01` |
| valid_to | DATE | 유효종료일 (NULL=무기한) | DATBI | `2026-12-31` 또는 NULL |
| last_po_date | DATE | 최근 발주일 | — | `2026-03-15` |
| last_po_price | NUMERIC(15,2) | 최근 발주 단가 | — | `1450.00` |
| last_po_id | UUID | 최근 발주 (FK: purchase_orders) | — | 가장 최근 PO 참조 |
| is_preferred | BOOLEAN | 우선 공급사 여부 | — | `TRUE` = 자동 소싱 시 우선 선택 |
| status | VARCHAR(10) | 상태 | — | `active`, `expired`, `blocked` |
| notes | TEXT | 비고 | — | `2026년 연간 계약` |
| created_at | TIMESTAMPTZ | 생성일시 | — | `2026-04-02 09:00:00+09` |
| updated_at | TIMESTAMPTZ | 수정일시 | — | `2026-04-02 09:00:00+09` |

> UNIQUE 제약: (parts_id, vendor_id, valid_from) WHERE status = 'active'
...
Transportation Management System. 위치, 운송사, 배차, 운송요청, 운송실행, 출고지시, OTIF 성과.
총 10개 테이블.
...
#### 5.4.10 tms.otif_records (OTIF 배송성과 기록)

> SAP TM Delivery Performance KPI | Freight Order별 On Time In Full 측정 결과

| 컬럼명 | 데이터타입 | 한글 설명 | SAP 대응 | 예시 |
|--------|-----------|-----------|----------|------|
| id | UUID | 고유 식별자 (PK) | — | `550e8400-...` |
| freight_order_id | UUID | 운송오더 (FK: freight_orders, UNIQUE) | — | 1:1 관계 |
| project_id | UUID | 프로젝트 (FK: projects) | — | 해당 배송의 프로젝트 |
| carrier_id | UUID | 운송사 (FK: carriers) | — | 배송 담당 운송사 |
| route_id | UUID | 루트 (FK: routes) | — | 배송 경로 |
| planned_delivery_date | DATE | 약속 납기일 | — | `2026-03-20` |
| actual_delivery_date | DATE | 실제 배송일 | — | `2026-03-21` |
| on_time | BOOLEAN | 정시 여부 | — | `TRUE` = actual ≤ planned |
| time_variance_days | INTEGER | 납기 차이(일) | — | `1` (음수=조기배송) |
| planned_qty | INTEGER | 약속 수량 | — | `500` |
| actual_qty | INTEGER | 실제 수량 | — | `500` |
| in_full | BOOLEAN | 완전납품 여부 | — | `TRUE` = actual ≥ planned |
| qty_variance | INTEGER | 수량 차이 | — | `0` (음수=부족) |
| otif_score | BOOLEAN | **GENERATED**: on_time AND in_full | — | `TRUE` = OTIF 달성 |
| measurement_date | TIMESTAMPTZ | 측정일시 | — | `2026-03-21 15:30:00+09` |
| measured_by | UUID | 측정자 (FK: users) | — | OTIF 기록한 직원 |
| failure_reason | TEXT | 미달성 사유 | — | `교통체증으로 1일 지연` |
| notes | TEXT | 비고 | — | |
| created_at | TIMESTAMPTZ | 생성일시 | — | `2026-03-21 15:30:00+09` |

> otif_score는 GENERATED ALWAYS AS (COALESCE(on_time, FALSE) AND COALESCE(in_full, FALSE)) STORED
...
| **mm** | 13 | purchase_requisitions, purchase_orders, purchase_order_items, goods_receipts, stock_movements, invoice_verifications, reservations, return_orders, quality_inspections, scrap_records, defect_catalogs, inspection_defects, purchasing_info_records |
| **wms** | 7 | warehouses, storage_types, storage_bins, batches, quants, inventory_count_docs, inventory_count_items |
| **tms** | 10 | locations, carriers, dispatch_schedules, transportation_requirements, freight_orders, logistics_releases, logistics_release_items, packaging_materials, routes, otif_records |
...
> 이 문서는 Supabase 마이그레이션 파일(002~018)을 기준으로 생성되었습니다.
> 실제 운영 DB(sap 스키마)와 TO-BE(6스키마)가 병존하고 있으며, 이 문서는 TO-BE 구조를 다룹니다.
> 018: Quick Wins (2026-04-02) — defect_catalogs, inspection_defects, purchasing_info_records, otif_records + 뷰 6개
