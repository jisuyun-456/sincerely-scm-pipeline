...
### 3.2 통합 적재 Wave 시스템화 (Axis 2)

**목표:** 분산 적재 → 통합 적재로 차량이용률 ↑ + 자체 기사 활용도 ↑

**Wave 정책:**
| Wave | 시간 | 대상 | 정책 |
|------|------|------|------|
| W1-AM | 09:00 | 이장훈 (오전 슬롯) | 수도권 소형 ≤3건 |
| W2-AM | 09:00 | 조희선 (오전 콘솔) | 수도권/인천 ≤3건 |
| W3-PM | 14:00 | 조희선 (오후 콘솔) | 수도권/인천 ≤3건 |
| W4-Day | 09:00 (기본) — 12:00 fallback | 박종성 (전국 wave) | 전국 ≤7~8건 (CBM 묶음). 정확 시간은 Open Decision 5 참조 |

**Wave 그룹화 알고리즘 (개념):**
1. 출하확정일 = today + 0 또는 1 인 shipment 추출
2. 지역(`Shipment.수령인_주소`) → 수도권/지방 분류
3. CBM 합산 → 각 wave capacity 매칭
4. 1순위 = 자체 기사 wave 우선 채움, 잔여만 외주

**구현 위치:** `dispatch_advisor.py` 갱신 + Airtable `Shipment.wave_group` 필드 신규 + `harness/dispatch/wave_planner.py` 신규
...
### Sub-Spec 3: 통합 적재 Wave 알고리즘 (3~4주)
- `harness/dispatch/wave_planner.py` — 출하확정일 + 지역 + CBM 기반 wave 그룹화
- Airtable `Shipment.wave_group` 필드 신규
- 자체 기사 wave 우선 채움 + 잔여 외주
...
5. **통합 적재 wave 시간창** — 현재 조희선 콘솔 시간이 정확히 09:00·14:00인가? 영업·CX팀과 *출고지시 발행 마감 시간* 협의
