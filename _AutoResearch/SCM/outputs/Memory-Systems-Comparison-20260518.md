...
**한 줄 결론 (v1.1 재구성)**: Hindsight와 OMEGA **둘 다 셀프호스트 완전 무료** (MIT / Apache-2.0). 가격이 결정 요인 아님. 핵심 갈림길은 ① Obsidian vault 통합 편의 vs ② 커뮤니티 트랙션·BEAM 1위. **결정은 한국어 30분 PoC로 양쪽 동시 테스트** (`scripts/poc_memory_systems.py`) 후 적중률·지연시간으로 확정.

| 항목 | 결정 |
|------|------|
| **공동 1순위 후보** | **Hindsight** (`pip install hindsight-all`) **+** **OMEGA** (`pip install omega-memory[server]`) — PoC 적중률로 최종 결정 |
| Use-case 가중치 | Obsidian vault 직접 통합 우선 → **OMEGA 유리** / 범용 agent memory + 검증된 트랙션 우선 → **Hindsight 유리** |
| 인프라 | 랩탑 only — Docker / Railway / Fly **불필요** (둘 다 단일 `pip install`) |
| 비용 | **셀프호스트 양쪽 모두 $0** (Hindsight MIT, OMEGA Apache-2.0). LLM API 비용은 임베더 선택 따라: OpenAI/Anthropic (소량 $) vs Ollama 로컬 ($0) |
| Python 요구사항 | **≥ 3.11** (Hindsight 명시). OMEGA도 3.10+ 권장 |
| 선결 조건 | **한국어 30분 PoC** (`scripts/poc_memory_systems.py` 자동화 완비) |
| 정책 가드 | Airtable 단일 진실원천 — 운영 데이터 (movement/mat_document/shipments) indexing 금지 |
| 다음 액션 | 사용자가 랩탑에서 PoC 스크립트 실행 → 결과 공유 → 본 보고서 v1.2로 최종 확정 |
...
> **v1.1 정정**: 설치 변형은 4종이다. 사용자 환경에 맞춰 선택.

| 변형 | 명령 | 용도 |
|------|------|------|
| **권장** | `pip install hindsight-all` | **out-of-box 전체 패키지** — 임베딩·rerank·DB 모두 포함, 기본 추천 |
| 슬림 | `pip install hindsight-all-slim` | 임베딩·rerank·DB를 외부 서비스로 위임 (이미 OpenAI/Anthropic 인프라가 있을 때) |
| 서버 | `pip install hindsight-api` | 서버 컴포넌트만 — 다른 곳에서 client 접속 |
| 클라이언트 | `pip install hindsight-client` | Python 클라이언트만 (서버는 별도 호스트) |
| uvx | `uvx hindsight-embed@latest configure` | zero-install, uv 사용 시 |
| npm | `npm i @vectorize-io/hindsight-all` | Node 환경용 |

**Python ≥ 3.11 필수**. Windows / macOS / Linux 네이티브 지원. embedded pg0 (Postgres) 자동 포함 — 별도 DB 설치 불필요. SQLite 모드 옵션 제공.

### 2.4 비용 구조 (v1.1 추가 — 사용자 핵심 관심사)

| 항목 | 비용 |
|------|------|
| **Hindsight 셀프호스트** | **$0** — MIT 라이센스, 사용 제한 없음, 텔레메트리 없음, 프로덕션 가능 |
| LLM API (임베딩+rerank) | 임베더 선택 따라: OpenAI text-embedding-3 (~$0.02/1M tokens) / Anthropic / Ollama 로컬 ($0) |
| Hindsight Cloud (관리형) | pay-as-you-go, free credits 제공, 고정 월정액 없음 |

→ **랩탑 셀프호스트 + Ollama 로컬 임베더 조합 시 완전 무료** ($0).

### 2.5 통합

- **MCP 서버 내장** — Claude Code, Claude Desktop, Cursor, Windsurf, Zed 즉시 연결
- **SDK 3종** — Python, TypeScript, Go
- **LLM 멀티 지원** — OpenAI, Anthropic, Gemini, Groq, **Ollama, LM Studio** (로컬 모델로 한국어 임베더 다양화 가능)
...
## 9. 최종 추천 — v1.1 재구성 (공동 1순위 + PoC 결정)

### 9.0 재구성 배경 (v1.0 → v1.1)

v1.0 보고서는 "Hindsight 1순위 + OMEGA 이주 경로" 구조였음. 사용자가 이를 검토한 결과 **Hindsight 셀프호스트가 완전 무료**(MIT, 사용 제한 0)임이 확인됨 → v1.0의 "이주 경로" 논리(향후 가격정책 리스크)가 약화. 가격은 이제 결정 요인이 아니며, **두 후보가 거의 동률**이 됨. 따라서 **PoC 결과로 최종 결정**하는 구조로 재구성.

### 9.1 두 후보의 동률 분석

| 차원 | 우위 | 근거 |
|------|------|------|
| 셀프호스트 비용 | **동률** | 둘 다 $0 (MIT vs Apache-2.0) |
| Python 설치 편의 | **동률** | 둘 다 단일 `pip install` (Docker 불필요) |
| LongMemEval | **OMEGA** | 95.4% vs 91.4% (4% 마진) |
| BEAM 10M | **Hindsight** | 64.1% 1위 (OMEGA 미발표) |
| 커뮤니티 트랙션 | **Hindsight** | 13.6k★ + VentureBeat |
| 거버넌스 안정성 | **OMEGA** | 재단 + Core 영구 OSS 약속 (Hindsight=VC 스타트업) |
| **Obsidian 통합** | **OMEGA** | 공식 플러그인 (Hindsight=커스텀 스크립트 필요) |
| 검색 아키텍처 다양성 | **Hindsight** | 4-병렬 + cross-encoder rerank |
| Mental Models 자동 | **Hindsight** | 메모리 누적 → 살아있는 문서 자동 재작성 |
| SDK 다양성 | **Hindsight** | Python/TS/Go 3종 (OMEGA = Python) |
| 한국어 임베더 유연성 | **Hindsight** | LLM 임베딩 (OpenAI 멀티링구얼) + Ollama 로컬 즉시 가능 |
| MCP | **동률** | 둘 다 지원 |

**합계**: Hindsight 5승 / OMEGA 3승 / 동률 3 → **수치상으로는 Hindsight 약간 우위**. 다만 OMEGA의 3승 중 Obsidian 통합·거버넌스는 *우리 use case에서 가중치가 높음*.

### 9.2 Use-case별 추천

| 사용 시나리오 | 추천 | 이유 |
|--------------|------|------|
| **`_AutoResearch/SCM/wiki/` Obsidian vault 직접 통합 우선** | **OMEGA** | 공식 플러그인 → zero-integration |
| **범용 agent memory + 검증된 트랙션 우선** | **Hindsight** | 13.6k★ + BEAM 1위 + SDK 3종 |
| **한국어 정확도가 결정적 (PoC 결과 의존)** | **PoC로 결정** | 둘 다 임베더 교체 가능 — 실측이 답 |
| **거버넌스 리스크 회피 (rug-pull 우려)** | **OMEGA** | 재단 + Apache-2.0 |
| **Mental Models 자동 재작성 매력** | **Hindsight** | OMEGA의 consolidation은 수동 트리거 |

### 9.3 결정 절차 — PoC 자동화 스크립트 사용

본 보고서와 함께 `scripts/poc_memory_systems.py` 자동화 스크립트 작성됨. 사용자 랩탑에서 실행하면 30분 내 객관적 적중률 데이터 확보.

```bash
# 랩탑 (사용자 직접 실행 — 본 컨테이너에서 PyPI 차단됨)
cd ~/sincerely-scm-pipeline
python3 -m venv .venv && source .venv/bin/activate
pip install hindsight-all omega-memory[server]

# 양쪽 동시 PoC (TMS-W18.md retain → 5개 한국어 쿼리 recall → 적중률)
python scripts/poc_memory_systems.py --json poc_results.json
```

스크립트가 수행하는 일:
1. `_AutoResearch/SCM/outputs/TMS-2026-W18.md` 내용을 양쪽에 retain/store
2. 5개 한국어 쿼리 (에이원 OTIF / 수도권 직배송 / 내부 소화율 60% / PT2429 / 다영기획)
3. top-3 결과의 키워드 포함 여부로 채점
4. side-by-side 적중률 + 지연시간 표 출력
5. Go/No-Go 자동 판정

### 9.4 Go / No-Go 매트릭스

| 시나리오 | Hindsight 적중률 | OMEGA 적중률 | 결정 |
|---------|-----------------|--------------|------|
| 한쪽 ≥ 4/5, 다른 쪽 < 4/5 | — | — | **이긴 쪽 1순위 도입** |
| 둘 다 ≥ 4/5 | — | — | **Obsidian 통합 우선 → OMEGA** (or 사용자 추가 의견) |
| 둘 다 2~3/5 | — | — | **임베더 교체 후 재시도** (Hindsight=OpenAI 임베딩 / OMEGA=bge-m3) |
| 둘 다 < 2/5 | — | — | **NO-GO** — 현재 grep 체계 유지 + Tier 0 메타데이터만 적용 |

### 9.5 Tier 0 (NO-GO 대안): 코드 0줄 메타데이터 보강

PoC 양쪽 실패 시 폴백 — 현재 마크다운 파일 상단에 frontmatter 추가:

```yaml
---
entities: [에이원, 한라케미칼, 다영기획]
products: [PT2429, PT0510]
themes: [carrier-OTIF, 휴면재고, 운임]
date: 2026-05-08
weeks: [2026-W18]
---
```

→ `grep -l 'PT2429' _AutoResearch/SCM/**/*.md` 같은 명령으로 메타 기반 검색 가능. SessionStart 훅에서 최근 frontmatter도 노출하도록 개선.
...
- **2026-05-18 v1.1** — 사용자 검토 후 재구성. 핵심 변경:
  - 컨테이너에서 PoC 실행 시도 → PyPI 차단 (`host_not_allowed`) 확인 → 랩탑용 자동화 스크립트 `scripts/poc_memory_systems.py` 작성으로 대체
  - 발견된 3개 오류 정정: ① pip 설치 변형 4종 명시 (hindsight-all 권장) ② Hindsight 셀프호스트 완전 무료 사실 §2.4에 추가 ③ Python ≥ 3.11 요구사항 추가
  - **결정 구조 재편**: "Hindsight 1순위 + OMEGA 이주 경로" → "공동 1순위 + PoC 결과로 결정". 셀프호스트 양쪽 무료 → 가격 결정요인 제거. Use-case 매트릭스 + Go/No-Go 매트릭스로 객관화.
- 2026-05-18 v1.0 — 초안 작성. 사용자 요청: "랩탑-only, 3-way 비교, 1순위 추천". 잠정 결정: Hindsight 1순위 + OMEGA 이주 경로.
