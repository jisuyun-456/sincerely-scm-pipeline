"""Phase E — 임가공출고요청 자동화 에이전트.

Trigger: sap.mat_document 중 movement_type='261'이고
         아직 sim_agent_event(임가공출고요청)가 없는 건에 임가공처 작업 요청 발송.
"""
...
def run() -> int:
    """Send OEM work requests for movement_type=261 documents. Returns count processed."""
    cfg = get_config()
    dry_run = cfg.dry_run
    ...
    for doc in to_process:
        ...
        total_qty = sum(abs(float(item.get("qty_signed", 0))) for item in items)

        message = f"SO {source_doc_id} | {total_qty:.0f}EA | 임가공처 CS-0001 작업 요청 발송"
...
if __name__ == "__main__":
    ...
    count = run()
    print(f"{AGENT_NAME}: {count} OEM request(s) sent")
    sys.exit(0)
