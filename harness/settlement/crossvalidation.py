...
KR_COORDS: dict[str, tuple[float, float]] = {
    ...
    "수원": (37.2636, 127.0286), "춘천시": (37.8813, 127.7298),
    "원주시": (37.3422, 127.9202), "강릉시": (37.7519, 128.8761),
...
def estimate_dest_coord(addr: str):
    ...
    Search order: specific name first, then administrative 구/시.
    """
    if not addr:
        return None

    # Try direct match of KR_COORDS keys in address
    for name, coord in KR_COORDS.items():
        if name in addr:
            return coord

    # Try extracting 구/시/군 from address with regex
